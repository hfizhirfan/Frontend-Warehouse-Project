      ...
      <div class="overflow-x-auto w-full pb-2">
        <table class="w-full text-left min-w-[800px] md:min-w-[900px]">
          ...
          <tbody class="text-sm divide-y divide-gray-100">
            <tr v-for="item in filteredHistory" :key="item.id" class="hover:bg-gray-50/80 transition-colors">
              <td class="px-4 py-3 md:px-6 md:py-4 whitespace-nowrap">
                <div class="font-bold text-gray-800 text-xs md:text-sm">{{ formatDate(item.created_at) }}</div>
                <div class="text-[10px] md:text-xs text-gray-500 font-medium mt-0.5">{{ formatTime(item.created_at) }}</div>
              </td>
              <td class="px-4 py-3 md:px-6 md:py-4 font-mono text-gray-600 text-[10px] md:text-xs">{{ item.waybill_number || 'Tidak Ada Dokumen' }}</td>
              <td class="px-4 py-3 md:px-6 md:py-4">
                <span class="font-medium text-gray-800 bg-gray-100 px-2 py-1 md:px-2.5 md:py-1 rounded-md text-[9px] md:text-[11px] border border-gray-200">{{ item.product.brand.name }}</span>
              </td>
              <td class="px-4 py-3 md:px-6 md:py-4 min-w-[150px]">
                <div class="font-bold text-gray-800 text-xs md:text-sm">{{ item.product.sku }}</div>
                <div class="text-[10px] md:text-xs text-gray-500 mt-0.5">{{ item.product.name }}</div>
              </td>
              <td class="px-4 py-3 md:px-6 md:py-4 text-right">
                <span class="font-bold text-green-600 bg-green-50 px-2 py-0.5 md:px-2.5 md:py-1 rounded-md border border-green-100 text-xs md:text-sm">+{{ item.quantity.toLocaleString('id-ID') }}</span>
              </td>
              <td class="px-4 py-3 md:px-6 md:py-4 text-gray-600 font-medium text-xs md:text-sm">{{ item.remark || '-' }}</td>
              <td class="px-4 py-3 md:px-6 md:py-4 flex justify-center">
                <span class="inline-flex items-center justify-center bg-green-50 text-green-700 px-2 py-1 md:px-2.5 md:py-1 rounded-md text-[10px] md:text-xs font-bold w-full max-w-[90px] md:max-w-[110px] border border-green-100">
...
const { data: apiResponse } = await useFetch('https://aa14-2400-9800-9b2-5623-7868-912c-7b31-a15e.ngrok-free.app/api/inbounds')
...
// Fungsi untuk memformat jam (contoh: 08:30 WIB)
const formatTime = (dateString) => {
  const date = new Date(dateString)
  return date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' }) + ' WIB'
}

// Fitur Filter (Pencarian & Hak Akses Brand)
const filteredHistory = computed(() => {
  if (!apiResponse.value?.data) return []

  let result = apiResponse.value.data

  // Filter berdasarkan Role Brand
  if (currentUser.value.role !== 'super_admin') {
    result = result.filter(item => item.product.brand.name === currentUser.value.assigned_brand)
  } else {
    if (globalSelectedBrand.value && globalSelectedBrand.value !== 'Semua Brand') {
      result = result.filter(item => item.product.brand.name === globalSelectedBrand.value)
    }
  }
  ...
  if (searchQuery.value) {
    ...
    result = result.filter(item => 
      ...
      (item.waybill_number && item.waybill_number.toLowerCase().includes(query))
    )
  }
