      ...
      <div class="w-full lg:w-1/3">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-6 lg:p-8 relative overflow-hidden">
          ...
          <form @submit.prevent="submitInbound" class="space-y-4 md:space-y-5 relative z-10">
            ...
            <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3 md:py-3.5 px-4 rounded-xl hover:bg-blue-700 transition-colors mt-2 md:mt-4 flex justify-center items-center shadow-md shadow-blue-200 text-sm md:text-base">
              <span class="material-symbols-outlined mr-2 text-[18px] md:text-[24px]">save</span> Simpan Stok
            </button>
          </form>
        </div>
      </div>
            ...
            <table class="w-full text-left min-w-[600px]">
              ...
              <tbody class="text-xs md:text-sm divide-y divide-gray-100">
                <tr class="hover:bg-gray-50/80 transition-colors">
                  <td class="px-4 py-3 md:px-6 md:py-4 text-gray-500 text-[11px] md:text-xs font-medium">08:30 WIB</td>
                  <td class="px-4 py-3 md:px-6 md:py-4">
                    <span class="font-medium text-gray-800 bg-gray-100 px-2 py-1 md:px-2.5 md:py-1 rounded-md text-[10px] md:text-xs border border-gray-200 whitespace-nowrap">CA SKIN GLOW</span>
                  </td>
                  <td class="px-4 py-3 md:px-6 md:py-4 font-mono text-gray-500 text-[11px] md:text-xs">CASM001</td>
                  <td class="px-4 py-3 md:px-6 md:py-4 text-right">
                    <span class="font-bold text-green-600 bg-green-50 px-2 py-1 md:px-2.5 md:py-1 rounded-md border border-green-100">+500</span>
                  </td>
                  <td class="px-4 py-3 md:px-6 md:py-4 text-gray-700 min-w-[120px]">Pabrik A</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const { data: apiResponse } = await useFetch('https://9d55-140-213-218-16.ngrok-free.app/api/products')
...
const form = ref({
  ...
  product_id: '',
  quantity: '',
  date: new Date().toISOString().split('T')[0], // Otomatis tanggal hari ini
  supplier: ''
})

// 4. Otomatisasi: Mengambil daftar Brand yang unik dari API Produk
...
// 5. Otomatisasi: Memfilter SKU berdasarkan Brand yang dipilih user di dropdown pertama
const filteredProducts = computed(() => {
  if (!apiResponse.value?.data) return []
  if (!form.value.brand_id) return [] // Kosongkan SKU jika Brand belum dipilih
  return apiResponse.value.data.filter(p => p.brand_id === form.value.brand_id)
})

const submitInbound = async () => {
  // Pastikan user sudah mengisi semua field
  ...
  try {
    ...
    const res = await $fetch('https://9d55-140-213-218-16.ngrok-free.app/api/inbounds', {
      method: 'POST',
      body: form.value
    })

    if (res.status === 'success') {
      alert('✅ Stok berhasil disimpan ke database!')
      
      // 2. Reset form agar kosong kembali
      form.value.product_id = ''
      form.value.quantity = ''
      form.value.supplier = ''
      
      // 3. Refresh data (opsional: agar dropdown stok terbaru terupdate)
      refreshNuxtData() 
      
      // Kirim user ke halaman riwayat untuk melihat hasilnya
      navigateTo('/inbound-history')
    }
  } catch (error) {
    console.error('Error saat simpan data:', error)
    alert('❌ Gagal menyimpan data. Pastikan server Laravel menyala dan konfigurasi CORS sudah benar.')
  }
}
</script>